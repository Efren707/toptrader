...
- **Account deletion cascades**: `holdings`, `transactions`, and the user's own token rows are deleted along with the `users` row, application-side (not a DB-level FK cascade). Originally decided as block-not-cascade in ADR 0047; reversed in [ADR 0048](../adr/0048-account-deletion-cascade.md) once it became clear blocking had no real exit for any user who had ever traded.
