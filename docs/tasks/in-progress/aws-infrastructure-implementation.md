...
- [x] Backend: SSH-key GitHub secret, SCP the built jar + `systemctl restart` (ADR 0006) — dedicated low-privilege `deploy` Linux user on EC2 (not `ec2-user`), keyed with its own ed25519 pair generated locally (private key never touches the server); `deploy` is a member of the `toptrader` group so it can overwrite `/opt/toptrader/app.jar` in place, and has exactly one NOPASSWD sudoers entry (`/usr/bin/systemctl restart toptrader.service`, no wildcard) — verified live that `deploy` can restart the service but is correctly denied `systemctl status`/`journalctl` (no matching sudoers rule). Private key stored as GitHub secret `EC2_DEPLOY_SSH_KEY`. `backend-ci` copies the repackaged jar to a fixed `app.jar` name and uploads it as a build artifact; new `deploy-backend` job downloads that artifact and uses `appleboy/scp-action` to copy it to `/opt/toptrader`.
...
- [x] Wire the deploy stage into the existing lint→test→build pipeline, gated on merge to `main` only (ADR 0016/0017) — satisfied by construction: both `deploy-frontend` and `deploy-backend` live in the same `.github/workflows/ci.yml`, each gated on `github.ref == 'refs/heads/main' && github.event_name == 'push'` plus their respective CI job actually succeeding (not skipped), no separate wiring step needed.
