...
**US-2 (Log in) is done.** Backend (`POST /auth/login`, `GET /auth/session`, inline brute-force lockout per **ADR 0025**, a `SecurityConfig` fix for a 403-vs-401 gap on unauthenticated requests) and frontend (`Login` component, `AuthService.checkSession()` via an app initializer, a minimal `Dashboard` placeholder, `authGuard`/`guestGuard` route guards) merged together in PR [#14](https://github.com/Efren707/toptrader/pull/14), 2026-07-20. **Next step:** start US-3 (Receive starting virtual cash — mostly already covered by US-1's `$500.00` grant at registration; worth a quick audit against `acceptance-criteria.md`, and note `Dashboard` doesn't display the cash balance yet, before moving to US-4). Mentor-mode coding collaboration continues per `CLAUDE.md`. (Phase 5's demo/showcase readiness item is still open and intentionally blocked until deploy — see `docs/guides/demo-showcase-readiness-outline.md`.)
