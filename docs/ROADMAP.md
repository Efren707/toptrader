...
**Email verification at signup is done and merged** (PR [#95](https://github.com/Efren707/toptrader/pull/95), design in [ADR 0037](./adr/0037-email-verification-at-signup.md)): no login gate — registration's existing auto-login/redirect is unchanged, verification only records `User.emailVerifiedAt` and sends a link, mirroring ADR 0036's password-reset token pattern. New `email_verification_tokens` table, `EmailVerificationService.verifyEmail`/`resendVerification` (enumeration-safe, invalidates prior outstanding tokens before reissuing), `POST /auth/verify-email`/`resend-verification` endpoints, new `RateLimitGroup.EMAIL_VERIFICATION`. Frontend: a reusable `NotificationService` (auto-dismissing toast, not verify-email-specific) shows a notice on successful registration, and a new `/verify-email` route auto-verifies on load with an inline resend form for the failed case. Full evidence lives in `docs/pre-deployment-checklist.md`'s Done section.

**Next up on the still-open checklist (quickest-to-largest):**
1. Logging and alerts in prod — zero logging framework exists yet; ADR 0008 plans the infra (logback → CloudWatch Logs, alarm → SNS email) but execution hasn't started.
2. Rollback strategy — the exception to "quickest-to-largest": the user wants an actual rollback plan instead of the current fix-forward posture, which needs its own dedicated session to design (likely revisiting ADR 0005/0006/0011/0017/0019).
