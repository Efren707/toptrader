...
**US-2 (Log in) is implemented, PR open.** Backend (`POST /auth/login`, `GET /auth/session`, inline brute-force lockout per **ADR 0025**, a `SecurityConfig` fix for a 403-vs-401 gap on unauthenticated requests) and frontend (`Login` component, `AuthService.checkSession()` via an app initializer, a minimal `Dashboard` placeholder, `authGuard`/`guestGuard` route guards) both done in one PR — [#14](https://github.com/Efren707/toptrader/pull/14), not yet merged. Full backend (9) and frontend (7) test suites passing, manual end-to-end browser verification done (lockout, no-enumeration, refresh-persistence, guard redirects both directions). **Next step:** once #14 merges, start US-3 (Receive starting virtual cash — mostly already covered by US-1's `$500.00` grant, worth a quick audit against `acceptance-criteria.md` before moving to US-4). Mentor-mode coding collaboration (CLAUDE.md) continues. (Phase 5's demo/showcase readiness item is still open and intentionally blocked until deploy — see `docs/guides/demo-showcase-readiness-outline.md`.)
