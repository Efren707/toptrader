...
1. **Email verification at signup — in progress**, on branch `feature/email-verification-at-signup`, design in [ADR 0037](./adr/0037-email-verification-at-signup.md) (no login gate; verification is informational only, mirrors ADR 0036's password-reset token pattern). Broken into 12 tracked steps:
   ...
   - [x] Frontend: register success verification notice — new `NotificationService` (`core/services`, signal-backed `message`/`show()`/`clear()`), shared rather than verify-email-specific since `Layout` renders it once for every authenticated route. `register.ts` calls `show(...)` right before its existing `/dashboard` redirect (redirect behavior itself unchanged); `layout.html` renders a dismissible banner between the navbar and router outlet. Copy is deliberately non-committal (mirrors `forgot-password`'s enumeration-safe tone) since the frontend has no way to know whether the verification email actually sent — `UserSummary` doesn't carry `emailVerifiedAt` or delivery status.
   - [ ] Frontend: verify-email route/component (success/expired/invalid + resend) — **next up.**
