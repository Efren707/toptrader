...
**Next up:** [#97](https://github.com/Efren707/toptrader/issues/97) logging framework in the backend (narrowed from its original scope — see note below), then [#98](https://github.com/Efren707/toptrader/issues/98) rollback strategy (needs its own dedicated design session).

Also this session: #97 got split in two. The CloudWatch log shipping + `StatusCheckFailed`→SNS alarm pieces (ADR 0008) can't actually be wired up yet — no EC2 instance exists, since AWS deployment hasn't started — so that half moved to a new issue, [#102](https://github.com/Efren707/toptrader/issues/102), blocked on EC2 provisioning. #97 now covers only the backend logging framework itself (Slf4j usage per ADR 0033's PII policy, a catch-all exception handler, and a `logback-spring.xml` with a local file appender for prod that #102's CloudWatch agent will later tail) — that part is doable now and is the actual next step.
